...
new ContentTypeImport<ImportedBackgroundFrame>(BackgroundContentType, "ase", (filename, then) => {
    console.log(`Invoking Aseprite to convert background "${filename}"...`)
    ...
    child_process.spawn("aseprite", ["--batch", filename, "--data", dataPath, "--list-tags", "--format", "json-array", "--save-as", path.join("Temp", "Content", "Imported", filename, `{frame}.png`)]).on("exit", status => {
        if (status != 0) Error(`Failed to invoke Aseprite to convert background "${filename}"`)
        ...
        fs.readFile(dataPath, "utf8", (err, dataJson) => {
            ...
            const preparedFrames: ImportedBackgroundFrame[] = []
            console.log(`Preparing frames from "${dataPath}"...`)
            for (const frame in data.frames) {
                PrepareBackgroundFramePng(path.join("Temp", "Content", "Imported", filename, `${frame}.png`), data.frames[frame].duration / 1000, path.join("Temp", "Content", "Imported", filename, `${frame}.trimmed.png`), preparedFrame => {
